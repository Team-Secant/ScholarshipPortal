...
router.post(
  ...
  async (req, res) => {
    ...
    if (!error.isEmpty()) {
      return res
        .status(400)
        .json({ success, error: "Please enter details correctly" });
    } else {
      try {
        ...
        if (studentemail) {
          return res
            .status(400)
            .json({ success, error: "Email already exist!" });
        } else {
          const verificationCode = Math.random().toString(36).substring(7);
          const verificationLink = `http://localhost:8000/verify?code=${verificationCode}`;

         var checkemail = await sendVerificationEmail(req.body.email, verificationLink);
...
router.get("/verify", async (req, res) => {
  try {
    ...
    const user = await student.findOne({ verificationCode: code });
  ...
  async (req, res) => {
    ...
    if (!error.isEmpty()) {
      ...
      try {
        ...
        return res
          .status(500)
          .json(acknowledged, "Internal Server Error occured!");
      }
    }
  }
);
