...
const EditUser = () => {
    ...
    const {fetchonescholarship,editscholarship} = useContext(scholarshipcontext)
    ...
    const loadScholarshipDetails = async () => {
        const response = await fetchonescholarship(id)
        setScholarship({name: response.name,description: response.description,minincome: response.minincome,mincgpa: response.mincgpa,sctype: response.sctype,active: response.active,award:response.award,lastdate:response.lastdate});     // response mein data ki property huti jis mein response ka data huta
    }
    ...
    return (
        <form onSubmit={EditUserDetails}>
            <Formgroup>
            <Typography variant="h2">Update Scholarship</Typography>
            <FormControl>
                <InputLabel htmlFor="my-input">Name of scholarship</InputLabel>
                <Input name='name' onChange={(e) => onValueChange(e)} value={scholarship.name}/>
            </FormControl>
            <FormControl>
                <InputLabel htmlFor="my-input">Description</InputLabel>
                <Input name='description' onChange={(e) => onValueChange(e)} value={scholarship.description}/>
            </FormControl>
            <FormControl>
                <InputLabel htmlFor="my-input">Minimum Income</InputLabel>
                <Input name='minincome' onChange={(e) => onValueChange(e)} value={scholarship.minincome}/>
            </FormControl>
            <FormControl>
                <InputLabel htmlFor="my-input">Minimum CGPA</InputLabel>
                <Input name='mincgpa' onChange={(e) => onValueChange(e)} value={scholarship.mincgpa}/>
            </FormControl>
            <FormControl>
                <InputLabel htmlFor="my-input">Scholarship Award in Rupees</InputLabel>
                <Input name='award' onChange={(e) => onValueChange(e)} value={scholarship.award}/>
            </FormControl>
            ...
            <div className="container d-flex justify-content-center flex-row my-5">
                <button className="btn btn-success d-flex justify-content-center align-items-center mx-4 w-50" type="submit">
                        ...
                        <p className="fs-6 m-0 mx-2"><AddIcon/>Save Changes</p>
                    </button>
